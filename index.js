$(document).ready(function(){
    ...
    $("img#contact_img").css("height",$("img#faq_img").height());
    };
    ...
    $("#main_product").css("height",$("#new_promotion").height());
    ...
    ocarousel.timer = 0;
    var oCarouselUl = ocarousel.getElementsByTagName("ul")[0];
    oCarouselUl.ontouchstart = function (e) {
        //clearInterval(ocarousel.timer);
        var startX = e.touches[0].pageX;
        var that = this;
        var curPos = this.offsetLeft;
        var timeStart = (new Date()).getTime();

        document.ontouchmove = function (e) {
            clearInterval(ocarousel.timer);
            var moveX = e.changedTouches[0].pageX;
            that.style.left = curPos+moveX-startX+'px';
            if(e.stopPropagation){
                e.stopPropagation();
            }else{
                e.cancelBubble = true;
        ...
        document.ontouchend = function (e) {
            ...
            var speed = (endX-startX)/(timeEnd-timeStart);
            //alert(speed);
            //alert(endX-startX);
            //alert(window.innerWidth);
            //右滑动
            if(endX-startX > window.innerWidth/2||speed>1){
                $("div.wrap #left").click();

            //左滑动
            }else if(endX-startX < -window.innerWidth/2||speed<-1){
                $("div.wrap #right").click();
            //没有超过一半滑动回当前位置
            }else{
                //$("ul#dot li");
                //找到当前显示的那张轮播图，点击对应的小原点
                var arr =[];
                var oDot = document.getElementById("dot");
                var oLi = oDot.getElementsByTagName("li");
                for(var i=0;i<oLi.length;i++){
                    if(oLi[i].offsetTop == -10){
                        arr.push(oLi[i]);
                    }
                }
                arr[0].click();
            ...
            clearInterval(ocarousel.timer);
            ocarousel.timer = setInterval(function(){
                $("div.wrap #right").click();
            },8000);
            document.ontouchmove = null;
            document.ontouchend = null;
    ...
    //自动播放
    //尝试使用原生js函数的方法,并且定时器加到对象上面，防止定时器全局变量干扰
    ...
    $(window).resize(function(){
       // $("img#contact_img").css("height",$("img#faq_img").height());
        ...
        $("#contact_img").css('height',$("#faq_img").height());
